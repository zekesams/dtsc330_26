"""This is our reusable classifier. A classifier should be able to be trained, 
    as well as to be able to make predictions. These function share the same data, 
    which suggests we should use:
"""
import numpy
import sklearn

# We can use a custom classifier, but most classifiers are 
# implemented well
...
class ReusableClassifier:
    def __init__(self, model_type: str = 'logistic_regression'):
        ...
        # : str is type hinting, which show sthat the input MSUT be a string
        # = 'logistic_regression' is the default argument
        # All shared variables must be defined in init
        """ Create a reusable classifier
        
        Args:
            model_type (str, optional): model type ocan be either
    ...
    def train(self, features, labels):
        if self.model_type == 'logistic_regession':
        ...
        # Different algorithms have different ways of evaluation the 
        # training.
    ...
    def predict(self, features):
        ...
        Args:
            features (_type_): Features MUST be teh same types as in 
    ...
    def assess(self, features, labels, split_data: bool = True, random_number: int = 42):
        ...
        # that's different from how well teh model performs
        # No statistician will accept it
        train_features, train_labels, test_features, test_labels = sklearn.model_selection.train_test_split(features, labels, test_size=0.2, random_state=random_number)
